document.addEventListener('DOMContentLoaded', function () {
    ...
    buttons.forEach(btn => {
        ...
        // preview on hover
        btn.addEventListener('mouseover', () => setActive(val));
        btn.addEventListener('focus', () => setActive(val));
        // restore after hover/focus out
        btn.addEventListener('mouseout', () => setActive(userRating));
        btn.addEventListener('blur', () => setActive(userRating));
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (avg && json.average !== undefined) {
                        avg.textContent = `Average: ${json.average} / 5 (${json.count})`;
                    }
                } else {
